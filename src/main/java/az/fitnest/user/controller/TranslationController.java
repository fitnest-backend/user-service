...
import java.util.List;
 ...
 public class TranslationController {
     ...
     @ApiResponses(value = {
             @ApiResponse(responseCode = "200", description = "Tərcümə uğurla yaradıldı",
                     content = @Content(schema = @Schema(implementation = Translation.class))),
             @ApiResponse(responseCode = "409", description = "Tərcümə artıq mövcuddur")
     })
     @PostMapping
     @PreAuthorize("hasRole('ADMIN')")
     public ResponseEntity<az.fitnest.user.dto.ApiResponse<Translation>> createTranslation(@RequestBody CreateTranslationRequest request) {
